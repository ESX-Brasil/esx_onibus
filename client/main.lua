...
function handleReturningBus()
    ...
    if playerDistanceFromCoords(coords) < Config.Markers.Size then
        ...
        TriggerServerEvent('esx_onibus:finishRoute', activeRoute.Payment)
        immediatelyEndRoute()
...
function payForEachPedLoaded(numberOfPeds)
    if numberOfPeds > 0 then
        ...
        TriggerServerEvent('esx_onibus:passengersLoaded', amountToPay)
...
function setUpLastStop()
    Log.debug('a próxima parada é a última, todos os peds devem partir')
    return 0, #pedsOnBus
end

function setUpAllStop()
    Log.debug('A próxima parada é Tudo, tudo deve ser feito')
...
function setUpSomeStop(freeSeats)
    ...
    Log.debug('próxima parada é alguns, decidiu aleatoriamente para spawn ' .. numberOfPedsToSpawn .. ' peds e partem ' .. numberDeparting)
...
function setUpNoneStop(freeSeats)
    ...
    Log.debug('a próxima parada é Nenhuma, decidindo aleatoriamente spawn ' .. numberOfPedsToSpawn .. 'peds')
...
function handleAbortRoute()
    if playerDistanceFromCoords(activeRoute.SpawnPoint) < Config.Markers.Size then
        ...
        if IsControlJustPressed(1, E_KEY) then
            ...
            TriggerServerEvent('esx_onibus:abortRoute', totalMoneyPaidThisRoute)
